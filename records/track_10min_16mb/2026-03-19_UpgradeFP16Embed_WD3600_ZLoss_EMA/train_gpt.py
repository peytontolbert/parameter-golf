...
class GPT(nn.Module):
    ...
    def forward(
        ...
        loss = F.cross_entropy(logits.float(), targets, reduction="mean", label_smoothing=label_smoothing)
        if z_loss_coeff > 0.0:
...
def main() -> None:
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            ...
            for micro_step in range(grad_accum_steps):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                    warmup_loss = model(x, y, args.label_smoothing, args.z_loss_coeff)
    ...
    while True:
        ...
        for micro_step in range(grad_accum_steps):
            ...
            x, y = train_loader.next_batch(args.train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y, args.label_smoothing, args.z_loss_coeff)
